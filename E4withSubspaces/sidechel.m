...
% step 5
     ...
     [A3 A1] = sidverg([[Di(:,(j-1)*r+1:j*r) Di(:,1:(j-1)*r)]; [Gami(:,(j-1)*r+1:j*r) Gami(:,1:(j-1)*r)]], [eye(m) zeros(m,(j-1)*m); zeros(n,m) iT*iO1]*[eye(j*m)-O*iO], O*T, j, m, r);
     ...
     X2 = [H(ik,:)*X(1:mb,:)+A3(ik,:)*R(ix(2,1):ix(2,2),ix(1,1):ix(6,2));R(ix(2,1):ix(2,2),ix(1,1):ix(6,2))];
     V = Y / X2;
     err = [Y-V*X2 iT*iO1*R(ix(7,1):ix(7,2),ix(7,1):ix(7,2))];
     ...
     V = R(ix(6,1):ix(6,2),ix(1,1):ix(6,2)) / X2;
     err = R(ix(6,1):ix(6,2),ix(1,1):ix(6,2))-V*X2;
...
%    Por qué se premultiplica por la matrix H?
     X2 = kron((H(ik,:)*X(1:mb,:))',P);
...
%    Por qué se premultiplica por H?
