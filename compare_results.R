# Script to compare MATLAB and R results after both have been executed
library(R.matlab)

compare_results <- function() {
  # Load MATLAB results
  if (file.exists("matlab_results.mat")) {
    matlab_results <- R.matlab::readMat("matlab_results.mat")
    
    # Load R results (save them from the R implementation if needed)  
    # For now, we'll use the r_result from the previous run and save it
    test_data <- create_test_data()
    theta <- test_data$theta
    din <- test_data$din
    r_result <- run_r_implementation(theta, din)
    
    # Save R results in MATLAB format to compare
    R.matlab::writeMat("r_results.mat", 
                      r_F = r_result$F, 
                      r_A = r_result$A, 
                      r_G = r_result$G, 
                      r_V = r_result$V)
    ...
    # Compare results
    cat("Comparing MATLAB and R implementations:\n")
    
    F_diff <- max(abs(r_result$F - matlab_results$F))
    A_diff <- max(abs(r_result$A - matlab_results$A))
    G_diff <- max(abs(r_result$G - matlab_results$G))
    V_diff <- max(abs(r_result$V - matlab_results$V))
    ...
    return(list(
      ...
      G_diff = G_diff,
      V_diff = V_diff,
      success = (F_diff < tolerance && A_diff < tolerance && G_diff < tolerance && V_diff < tolerance)
    ))
  } else {
    cat("MATLAB results file not found. Please run MATLAB implementation first.\n")
    return(NULL)
  }
}

# Define helper functions again in case this is run in a new session
source("e4init.R")
source("e4gthead.R") 
source("vecss.R")
source("cholp.R")
...
# Create helper functions that were defined in the test_conversion.R script
